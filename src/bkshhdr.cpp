...
/// プラグインの説明
static char Description[] = "Show headers with other windows";
...
DLLEXPORT int WINAPI BKC_OnOpenCompose(HWND hWnd, int nMode
                             /* See COMPOSE_MODE_* in BeckyApi.h */ )
{
    // Always return 0.
    return 0;
}
...
DLLEXPORT int WINAPI BKC_OnOutgoing(HWND hWnd, int nMode
                              /* 0:SaveToOutbox, 1:SaveToDraft, 2:SaveToReminder */)
{
...
DLLEXPORT int WINAPI BKC_OnKeyDispatch(HWND hWnd,
                             int nKey /* virtual key code */ ,
                             int nShift
                             /* Shift state. 0x40=Shift, 0x20=Ctrl, 0x60=Shift+Ctrl, 0xfe=Alt */
    )
{
...
DLLEXPORT int WINAPI BKC_OnRetrieve(LPCTSTR lpMessage /* Message source */ ,
                          LPCTSTR lpMailID /* Mail ID */ )
{
    // Always return 0.
    return 0;
}
...
DLLEXPORT int WINAPI BKC_OnPlugInInfo(LPBKPLUGININFO lpPlugInInfo)
{
     ...
     */
    strcpy(lpPlugInInfo->szPlugInName, "Show Header plug-in");
...
DLLEXPORT int WINAPI BKC_OnDragDrop(LPCSTR lpTgt, LPCSTR lpSrc, int nCount,
                          int dropEffect)
{
    /*
...
DLLEXPORT int WINAPI BKC_OnBeforeFilter(LPCSTR lpMessage, int *lpnAction,
                              char **lppParam)
{
    /*
